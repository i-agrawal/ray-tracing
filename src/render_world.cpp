...
Object* Render_World::Closest_Intersection(const Ray& ray, Hit& hit)
{
    ...
    for (Object* object : objects)
        object->Intersection(ray, hits);

    hit = {.object = 0, .t = std::numeric_limits<int>::max()};
    for (Hit h : hits) {
        if (h.t > small_t && h.t < hit.t)
            hit = h;
    }
...
{
    Ray ray(camera.position, camera.World_Position(pixel_index) - camera.position); // TODO: set up the initial view ray here
...
vec3 Render_World::Cast_Ray(const Ray& ray,int recursion_depth)
{
    ...
    if (Closest_Intersection(ray,hit) && recursion_depth <= recursion_depth_limit) {
        point = ray.Point(hit.t);
        normal = hit.object->Normal(point);
        shading = hit.object->material_shader;
    }
    color = shading->Shade_Surface(ray, point, normal, recursion_depth+1, hit.ray_exiting);
